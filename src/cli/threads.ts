...
/**
 * `bbchat threads` — list the current project's threads with their ids, i.e. the
 * `thr_...` values for `bbchat <thread-id>`. Read-only; does not create a project,
...
export async function runThreads(env: NodeJS.ProcessEnv = process.env): Promise<number> {
  ...
  if (!project) {
    throw new BbchatError(
      ...
      'Start one here with:  bbchat new "your first task"',
    );
  }

  const rows = renderThreadList(await listThreads(sdk, project.id));
  const lines: string[] = [`Threads in ${project.name} (open with: bbchat <id>):`, ""];
  if (rows.length === 0) {
    lines.push("  (no threads yet — start one with: bbchat new)");
  }
