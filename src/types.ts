...
export interface VchConfig {
  ...
  /** Whether `vch` may launch BB via `startCommand` when the server is unreachable. */
  autoStart: boolean;
}

/** A config with any subset of fields present (file contents, env overrides). */
export type PartialVchConfig = Partial<VchConfig>;
